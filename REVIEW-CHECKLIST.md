...
- [ ] N/A: Auth-protected routes and actions were tested while logged out. This MVP has no authentication or protected routes.
- [ ] N/A: Rate limiting (or equivalent abuse protection) considered for public endpoints. This MVP has no public backend endpoints.

## Draw Integrity
- [ ] Confirmed winner is never eligible again.
- [ ] Absent participant is never eligible again.
- [ ] At most one pending winner exists at any time.
- [ ] Current prize cannot advance while pending.
- [ ] Official winner is persisted before reveal animation.
- [ ] Animation/UI never selects or mutates the official winner.
- [ ] Refresh/resume restores the exact same pending winner.
- [ ] Event completes with exactly six confirmed prize winners.
